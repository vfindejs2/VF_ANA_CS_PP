...
| **AR-02** | Model Zóny — geometrie, Région a Zóna položky        | Atribut `geometrie` odebrán z business DS Zóny; zůstává jako technická/odvozená hodnota pro vizualizaci. Région **není** samostatnou entitou — v PP se přenáší jako textový atribut `Název regionu` přímo na Zóně (v HEN je Region uživatelský textový číselník, samostatný číselník v PP proto nevzniká). Adresní definice zóny přes entitu `Zóna položky` je odložena na budoucí etapu (v E1 se `Zóna položky` nezavádí; v dalších etapách bude řešena společně s vazbami Adresa → Zóna a Stanoviště → Zóna). | Zóna                                                                   |
...
| **AR-05** | Synchronizace nových entit CS — příjem změnových dat | Nové entity CS (Okruh, Rozvrh, Kalendar, Zóna) nejsou součástí stávajícího MDB kontraktu 202ucxx. Do PP budou plněny příjmem změnových dat z HEN, typicky přes REST API nebo samostatný import při inicializaci. Detailní integrační mechanismus není předmětem tohoto dokumentu.                                                                                                                        | Okruh, Rozvrh, Kalendar, Zóna                                          |
| **AR-06** | Editační režim synchronizovaných entit CS              | V Etapě 1 PP nové synchronizované entity a jejich vazby pouze eviduje a zobrazuje. Editace v aplikaci není povolena. Platí to i pro vazbu `RPO_Okruh_Rozvrh`.                                                                                                                                                                                                                                                                | Okruh, Rozvrh, Kalendar, Zóna, RPO_Okruh_Rozvrh                        |
...
- `RPO_Okruh_Rozvrh` je temporální vazební entita konsolidující přiřazení Okruh + Rozvrh k RPO. RPO může mít v jednom okamžiku více aktivních vazeb; za jejich správu odpovídá HEN (OB-03 potvrzen).
- Vazba RPO ↔ Nádoba je v PP vedena přes existující vazební tabulku `container_order_item_assignment` (beze změn v E1; OB-04 rozhodnuto).
- `Provozovna` je společný organizační kontext pro Okruh, Rozvrh a Zónu; odpovídá HEN konceptu Útvar/Stredisko (OB-02 potvrzen).
...
- `Zóna` je synchronizována z HEN; PP ji v E1 pouze eviduje a zobrazuje. Région je textový atribut Zóny (`Název regionu`). `Zóna položky` je v E1 vynechána — bude řešena v dalších etapách společně s vazbami Adresa → Zóna a Stanoviště → Zóna (AR-02).
- `Skupina odpadu` se **nesynchronizuje z HEN**; v PP se určuje na základě `Druhu odpadu` a aplikačních pravidel. Mapování je globální bez vazby na provozovnu (OB-05 potvrzen).
...
| Základní dny svozu | HEN                | Stávající — MDB → sync služba → API PP,***integrační DB je rozšířena o HEN zkrácený název dne svozu***                                                          | HEN → PP | 202ucxx                                                                                        |
...
| Skupina odpadu       | PP                 | Odvození v PP z Druhu odpadu v navazujícím kroku synchronizace RPO / Položky objednávky z MDB                                                                             | interní  | 202uc10 +***aplikační pravidlo PP nastavení skupiny odpadu dle mapování na druh odpadu*** |
| Adresa               | HEN                | Stávající — MDB → sync služba → API PP                                                                                                                                  | HEN → PP | 202ucxx +***aplikační pravidlo geokódování adres***                                       |
...
| **Zóna**            | HEN                | **Příjem změnových dat z HEN** / samostatný import (AR-05). Region jako textový atribut Zóny (`Název regionu`, AR-02).                                                                                                  | HEN → PP | Nový UC (viz OB-07)                                                                           |
...
2. **Nové entity CS se plní příjmem změnových dat z HEN.** Okruh, Rozvrh, Kalendar, Zóna nejsou součástí stávajícího MDB kontraktu 202ucxx. Do PP se plní příjmem změnových dat z HEN nebo samostatným importem při inicializaci (AR-05).
...
Seskupení druhů odpadu pro plánování a výběr RPO. V E1 je mapování Druh odpadu → Skupina odpadu globální (bez vazby na provozovnu); odvozování probíhá na úrovni aplikační logiky PP. Kompletní dokumentace: [Datový slovník → Obecné → Skupina odpadu](https://confluence.radium.cz/pages/viewpage.action?pageId=88033012).
...
| Atribut        | Datový typ        | Rozsah | Asociace       | Povinnost | Popis                                                                                                                            | Persistentní reference | Poznámka                                                                                                                                                                        |
| ---------------- | -------------------- | -------- | ---------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Skupina odpadu | Reference          | –     | Skupina odpadu | Ne        | Fallback skupina odpadu — platí pouze pokud nádoba nemá přiřazené RPO (přes `container_order_item_assignment`) nebo přiřazené RPO skupinu odpadu nemá.          | Ano                     | **Řídicí hodnota je vždy ze skupiny odpadu přiřazeného RPO** (pokud existuje a má skupinu odpadu vyplněnu). Přímé zadání povoleno pouze bez RPO nebo při prázdné skupině na RPO. |
...
Nová entita pro CS. Geografické seskupení RPO — pomocný plánovací a filtrační kontext. V Etapě 1 je zdrojem pravdy HEN; PP zónu pouze eviduje a zobrazuje, editace v aplikaci není povolena. Region se přenáší jako textový atribut `Název regionu` přímo na Zóně (AR-02). Atribut `geometrie` je odvozenou vizualizační hodnotou a není součástí business DS (AR-02).
...
| Provozovna               | Reference                    | –     | Provozovna   | Ano       | Provozovna, k níž zóna patří. Odpovídá HEN konceptu Útvar/Stredisko.                                | Ano                     | –                                                             |
| Název regionu           | Řetězec                    | 255   | –            | Ne        | Textový název regionu přenášený z HEN. Upřesňující klasifikace zóny — v HEN je Region uživatelský textový číselník. | –                      | –                                                             |
...
| -------------------------- | ------------------------------ | -------- | -------------- | ----------- | ---------------------------------------------------------------------------------------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Identifikátor           | Číslo (nezáporné, celé) | –     | –           | Ano       | Primární klíč záznamu.                                                                  | –                      | Jedná se o systémový atribut.                                                                           |
| RPO                      | Reference                    | –     | RPO          | Ano       | Revize položky objednávky přiřazená do okruhu a rozvrhu.                                | Ano                     | RPO může mít v jednom okamžiku více aktivních vazeb RPO_Okruh_Rozvrh. Za založení nové nebo ukončení aktuální časové verze odpovídá HEN (OB-03 potvrzen). |
...
| Gap # | Oblast                                                                      | Stav                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1     | Zóna položky — chybějící entita                                       | **Odloženo na budoucí etapu** — v E1 se `Zóna položky` nezavádí. Adresní definice zóny bude řešena v dalších etapách společně s vazbami Adresa → Zóna a Stanoviště → Zóna (AR-02).                                                                                                                                                                                                                                                          |
| 2     | Région — chybějící entita                                              | **Rozhodnuto** — Région není samostatnou entitou PP. V HEN je Region uživatelský textový číselník; v PP se přenáší jako textový atribut `Název regionu` přímo na Zóně (AR-02).                                                                                                                                                                                                                                                                  |
...
| 5     | Zóna — geometrie vs. adresní definice                                    | **Zapracováno** — `geometrie` odebrána z DS (AR-02); `Zóna položky` odložena na budoucí etapu; Region jako textový atribut `Název regionu` přímo na Zóně                                                                                                                                                                                                                                                                                        |
...
| 10    | Zóna → Région a Útvar — chybějící vazby                             | **Rozhodnuto** — Région je textový atribut Zóny (`Název regionu`); `provozovna_id` PP odpovídá HEN konceptu Útvar/Stredisko (OB-02 potvrzen).                                                                                                                                                                                                                                                                                                   |
...
| OB-02 | Provozovna na entitách Okruh/Rozvrh/Zóna                                 | **Potvrzeno** — `provozovna_id` PP odpovídá HEN konceptu Útvar/Stredisko.                                                                                                                                                                                                                                                                                                                                                                            |
| OB-03 | Kardinalita a historie vazby `RPO_Okruh_Rozvrh`                           | **Potvrzeno** — RPO může mít v jednom okamžiku více aktivních vazeb RPO_Okruh_Rozvrh. Za založení nové nebo ukončení aktuální časové verze odpovídá HEN.                                                                                                                                                                                                                                                                                  |
| OB-04 | Kolize `RPO → Nádoba` — přímý FK vs. `container_order_item_assignment`  | **Rozhodnuto** — vazba RPO ↔ Nádoba je vedena přes existující `container_order_item_assignment` (beze změn v E1). Přímý FK `Nádoba.rpo_id` se nezavádí.                                                                                                                                                                                                                                                                                    |
| OB-05 | `Skupina odpadu → Provozovna`                                             | **Rozhodnuto** — v E1 nemá Skupina odpadu vazbu na provozovnu. Mapování Druh odpadu → Skupina odpadu je globální; odvozování probíhá na úrovni aplikační logiky PP.                                                                                                                                                                                                                                                                       |
...
| OB    | Oblast                                                                                                                                                                                                                                                                                             | Priorita  |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| OB-07 | Detailní integrační analýza HEN → PP pro nové synchronizované entity — doplnit datové kontrakty a mapování pro Okruh, Rozvrh, Kalendar a Zónu. Součástí analýzy je i potvrzení technického mechanismu příjmu změnových dat a případného bulk-load při inicializaci. | Vysoká   |
...
| 9. 4. 2026 | Vladislav Findejs | Zapracování rozhodnutí z review v2: (1) Région odebráno jako samostatná entita — Region je textový atribut Zóny (`Název regionu`); (2) `Zóna položky` odložena na budoucí etapu (řeší se spolu s vazbami Adresa/Stanoviště → Zóna); (3) vazba RPO ↔ Nádoba vedena přes `container_order_item_assignment` (OB-04); (4) RPO může mít více aktivních vazeb RPO_Okruh_Rozvrh — správa na straně HEN (OB-03); (5) Skupina odpadu bez vazby na provozovnu, mapování globální (OB-05); (6) potvrzeno — `provozovna_id` = Útvar/Stredisko HEN (OB-02). AR-02, AR-05, AR-06 aktualizovány. OB-07 zůstává otevřený. |
